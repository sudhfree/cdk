...
from api.services.LogQueueService import LogQueueService
import api.config.AppSettings as _settings
import sys
...
class IhmBaseLogger(object):
    ...
    def __init__(self, settingsIn: _settings.AppSettings = None, **kwargs):
        if IhmBaseLogger._logger != None:
            ...
            if settingsIn == None:
                ...
                settingsValues = settingsIn.dict()
                self.queueService = LogQueueService(settingsIn)
                self.DEFAULT_LOG_LEVEL = settingsIn.log_level
                self.DEFAULT_APPLICATION_NAME = settingsIn.application_name
                self.DEFAULT_COMPONENT_NAME = settingsIn.component_name
