...
import src.base_invoice_adjustment.config.AppSettings as _appsettings


class BIAStack(core.Stack):
    ...
    def __init__(self, scope: core.Construct, id: str, deploy_settings, **kwargs) -> None:
        ...
        self.app_settings = _appsettings.AppSettings(_env_file=self.deploy_settings.bia_app_config_path,
        ...
        bia_event_queue = self.createAdjustmentQueue()

        # 2. Lambda Construct - create Lambda function, security policy and roles.
        lambda_construct = self.createLambdaFunctionAndRole(bia_event_queue)
        return
    ...
    def getBaseName(self):
        ...
        _baseName = self.deploy_settings.environment + \
                    ...
                    self.deploy_settings.bia_application_name
        # No spaces allowed
        _baseName = _baseName.replace(' ', '')
        return _baseName

    def createAdjustmentQueue(self):
        sqs_construct_name = self.environment_name + self.name_spacer + self.deploy_settings.bia_application_name + self.name_spacer + "queue"
        sqs_name = sqs_construct_name
        dlq_construct_name = self.environment_name + self.name_spacer + self.deploy_settings.bia_application_name + self.name_spacer + "dlq"
        dlq_name = dlq_construct_name
        bia_event_DLQ = _sqs.Queue(self, dlq_construct_name, queue_name=dlq_name)
        bia_event_queue = _sqs.Queue(self, sqs_construct_name, queue_name=sqs_name,
                                     visibility_timeout=core.Duration.minutes(30),
                                     dead_letter_queue={'queue': bia_event_DLQ, 'max_receive_count': 5},
        ...
        filter_mapping = {"isAdjustment": _sns.SubscriptionFilter(conditions=["True"])}
        #filter_mapping = {"isAdjustment": _sns.SubscriptionFilter.string_filter(blacklist=["False"])}
        notification_topic.add_subscription(
            _subscriptions.SqsSubscription(queue=bia_event_queue,
                                           raw_message_delivery=True,
                                           filter_policy=filter_mapping
                                           ))
        return bia_event_queue

    def createLambdaFunctionAndRole(self, bia_event_queue):
        ...
        lambda_name = self.deploy_settings.environment + \
                      self.deploy_settings.aws_resource_name_spacer + \
                      self.deploy_settings.bia_application_name + "-lambda"
        ...
        lambda_exec_role = _iam.Role(self, self.deploy_settings.environment + \
                                     ...
                                     role_name=self.deploy_settings.environment + \
                                               self.deploy_settings.aws_resource_name_spacer + \
                                               self.deploy_settings.bia_application_name + "-role")
        ...
        lambda_construct = _lambda.Function(
            ...
            code=_lambda.Code.asset(f'../src/base_invoice_adjustment'),
            handler='invoice_adjustment.lambda_handler',
        ...
        lambda_construct.add_event_source_mapping(id=self.environment_name + self.name_spacer +
                                                  ...
                                                  event_source_arn=bia_event_queue.queue_arn)
